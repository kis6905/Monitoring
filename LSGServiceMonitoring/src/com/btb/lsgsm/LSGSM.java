...
import com.btb.lsgsm.netty.MMMClientHandler;
import com.btb.lsgsm.vo.Mail;
...
public class LSGSM {
	...
	public static final Logger logger = Logger.getLogger("LSGSM"); // thread-safe
	...
	private static boolean setupServer() {
		try {
			lsgAddr = Config.getLsgAddr();
			gwUrl = Config.getGwUrl();
			commandSet = Config.getCommandSet();
			serviceInfo = Config.getServiceInfo();
			terminalInfo = Config.getTerminalInfo();
	...
	private static void checkService() {
		...
		try {
			...
			conn.setRequestProperty("Command-Set", commandSet);
			conn.setRequestProperty("Service-Info1", serviceInfo);
			conn.setRequestProperty("Terminal-Info1", terminalInfo);
			conn.setRequestProperty("User-Info1", userInfo);

			DataOutputStream writer = new DataOutputStream(conn.getOutputStream());
			writer.write(bodyStr.getBytes());
			writer.flush();
			writer.close();
