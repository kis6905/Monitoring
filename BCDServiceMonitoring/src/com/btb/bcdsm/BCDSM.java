...
import com.btb.bcdsm.netty.MMMClientHandler;
import com.btb.bcdsm.vo.Mail;

/**
 * 서비스 확인은 30분 마다 한다.
 * SIPS에 서비스 상태 요청 -> 장애일 경우 Monitoring Mail Manager(MMM)에 메일을 보내라고 요청한다.
...
public class BCDSM {
	...
	public static final Logger logger = Logger.getLogger("BCDSM"); // thread-safe
	
	private static String mmmIp = null;
	private static int mmmPort;
	
	private static String sipsAddr = null;
	private static String monitoringUrl = null;
	private static String makerId = null;
	...
	private static boolean setupServer() {
		try {
			sipsAddr = Config.getSipsAddr();
			monitoringUrl = Config.getMonitoringUrl();
			makerId = Config.getMakerId();
	...
	/**
	 * 현재 시간이 21:00:00 ~ 24:00:00 또는 00:00:00 ~ 04:30:00 사이면 false, 아니면 true
	 */
	public static boolean isCheckTime() {
		...
		if (nowTime.compareTo(Constants.EXCLUDE_FROM_TIME) >= 0 && nowTime.compareTo(Constants.TWENTYFOUR_HOUR) < 0 ||
			nowTime.compareTo(Constants.ZERO_HOUR) >= 0 && nowTime.compareTo(Constants.EXCLUDE_TO_TIME) < 0)
			return false;
	...
	private static void checkService() {
		if (isCheckTime()) {
			try {
				...
				URL url = new URL(sipsAddr + monitoringUrl);
				...
				String bodyStr = "{\"makerId\": \"" + makerId + "\", \"password\": \"" + password + "\", \"serverIp\": \"" + serverIp + "\"}";
				...
				if (responseCode == 200) { // 정상
				...
				else if (responseCode == 400) { // 파라미터 에러
					...
					int errorCode = (Integer) resJson.get("errorCode");
					String invalidParam = (String) resJson.get("invaildParam");
					logger.info("errorCode: " + errorCode + ", invalidParam: " + invalidParam);
